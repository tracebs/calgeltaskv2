...
define('AMOCRM_LOGIN', 'shop24@calgel.ru');
define('AMOCRM_SUBDOMAIN', 'calgelacademy');
define('AMOCRM_API_KEY', '3dc13b3f69294ffdaef2abf884d1d908');
...
function fncAmocrmTaskList(
    ...
    curl_setopt($curl,CURLOPT_SSL_VERIFYPEER,0);
    curl_setopt($curl,CURLOPT_SSL_VERIFYHOST,0);
    curl_setopt($curl,CURLOPT_HTTPHEADER,array('IF-MODIFIED-SINCE: '.$strsince));
...
function fncAmocrmTasksUpdate(
    ...
    $tasks['request']['tasks']['update'] = $arrTasksUpdate;
...
function fncAmocrmRemoveTasks($strCookieFile,$stryesterd,$contactid,$tasktypeid) {		
	$countFound = 0; //число найденных для update контактов
	$countUpdated = 0; //число update контактов
    ...
    if (
		...
		//echo "point27-1";
		//3й параметр - id сотрудника для которого нужно получить контакты
		$arrAmocrmTaskList = fncAmocrmTaskList(
			AMOCRM_SUBDOMAIN,
			$strCookieFile,
			$stryesterd,
			'type=contact&element_id='.$contactid
		);
		if (
			! $arrAmocrmTaskList['boolOk']
		) {
		
		} # if
		else {
			//status1 = задача завершена
			//Забег в цикле по задачам и простановка выполнена тем который стухли
			
			$arrTaskList = $arrAmocrmTaskList['arrResponse']['tasks'];
			$i27 = 0;
			$arrstack = array();
			foreach($arrTaskList as $subtask) {	
				//var_dump($subtask);
				//echo "<br>";
				//and ($subtask['complete_till'])
				$tillint = (int)$subtask['complete_till'];
				if (($subtask['task_type']==$tasktypeid) and ($subtask['status']=='0') and (time()>$tillint) ) {
					$strid27 = "".$subtask['id'];
					array_push($arrstack,$strid27);
					//echo "!!!!!!!!!!!!!<br>";
					//пишем id задачи в массив
					$i27 = $i27+1;
			...
			if (count($arrstack)>0) {	
				//нужен апдейт
				foreach($arrstack as $stackval) {
					//echo "<br>id:".$stackval;
					$arrAmocrmTasksUpdate = fncAmocrmTasksUpdate(
						AMOCRM_SUBDOMAIN,
						$strCookieFile,
						array (
							array (
								'id' => $stackval, # id задачи								
								'status' => '1',
								'last_modified' => time(),
							),
						)
					...
					if (
						! $arrAmocrmTasksUpdate['boolOk']
					) {
					# $arrAmocrmTasksCreate['strErrDevelopUtf8']
					} # if
					else 
					{
						//var_dump($arrAmocrmTasksUpdate['arrResponse']);
						$countUpdated = $countUpdated + 1;
					}
				}
			}
			//var_dump($arrTaskList);
			$countFound = $i27;
		}	
	}
	
	return "Found ".$countFound." updated ".$countUpdated;
	//return $idNextResponsible;
}
